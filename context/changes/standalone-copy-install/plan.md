...
- [ ] 1.3 Test suite passes, incl. the new copy-mode suite and every existing test unchanged: `npm test`
- [ ] 1.4 `npm pack --dry-run --json` still lists only `dist/`, `skills/`, `rules/`, `bin/`, `README.md`, `package.json`

#### Manual

- [ ] 1.5 Scratch dir with no `package.json`: `install --copy` creates a real `.claude/skills/code-review/SKILL.md`, a `CLAUDE.md` block, a per-file manifest, and no `.npmrc`; re-run leaves `git status` clean
- [ ] 1.6 On Windows copy mode needs no Developer Mode / elevation
